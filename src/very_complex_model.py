...
FINAL_MODEL_NAME = 'final_model_god_v6_gpu_optimized.joblib'
LOG_FILE = 'training_log_v6_gpu.txt'
...
class TorchMLP(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        # Приводим к float32 для PyTorch
...
def get_params(trial, model_name):
    if model_name == 'xgb':
        ...
        p = {
            ...
            'l2_leaf_reg': trial.suggest_float('l2_leaf_reg', 1e-4, 15, log=True),
...
def obj_generic(trial, X, y, w, model_name, obj_type):
    # Приводим к float32 для PyTorch/GPU совместимости
...
class HybridModelGod:
    def __init__(self, p_lgbm_q, p_lgbm_m, p_xgb_q, p_xgb_m, p_cat_q, p_cat_m, p_mlp):
        ...
        self.mlp_meta_model = TorchMLP(**p_mlp)
    ...
    def fit(self, X_raw_full, y_full, weights_full):
        ...
        # Для скорости преобразуем один раз
...
if __name__ == '__main__':
    ...
    # [FIX] Оборачиваем в Series и возвращаем индексы, чтобы работал .loc
    ...
    log.info("--- 2. Optuna Search (GPU Mode) ---")
    ...
    log.info("🔎 Запуск поиска для DEEP MLP (PyTorch + CUDA)...")
    p_mlp = run_optuna_local('optuna_mlp_v6_gpu', lambda t: obj_generic(t, X_opt, y_opt, w_opt, 'mlp', 'm'),
                             N_TRIALS_MLP)

    log.info("\n--- 3. Сборка HybridModelGod (Balanced High Accuracy) ---")
