...
# ==========================================
# 2. ОСНОВНОЙ ПАЙПЛАЙН (СВОБОДНАЯ МОДЕЛЬ + ТОРМОЗА)
# ==========================================
def train_quantile_models(X_train, y_train, X_test, y_test):
    # 🛠️ СПЛИТ ДЛЯ РАННЕЙ ОСТАНОВКИ (Хронологически, последние 10%)
    split_idx = int(len(X_train) * 0.9)
    X_tr, X_val = X_train.iloc[:split_idx], X_train.iloc[split_idx:]
    y_tr, y_val = y_train.iloc[:split_idx], y_train.iloc[split_idx:]

    # 🔥 ОСЛАБЛЕННЫЕ КАНДАЛЫ + БАЗА OPTUNA 🔥
    lgbm_params = {
        'learning_rate': 0.02,
        'max_depth': 10,  # Даем возможность изгибаться
        'num_leaves': 90,  # Оптимальная гибкость
        'min_child_samples': 300,  # [!] ОСЛАБИЛИ КАНДАЛЫ (Было 600). Теперь коридор будет облегать точки!
        'n_estimators': 2000,  # Запас хода (Остановим раньше)
        'reg_alpha': 0.1,
        'reg_lambda': 0.1,  # [!] УБРАЛИ БЕТОН (Было 5.0)
        'subsample': 0.8,  # Берем 80% данных на дерево (защита от зубрежки)
        'subsample_freq': 1,
        'random_state': 42,
        'n_jobs': -1
    }

    print("-> 1. Обучение моделей с Early Stopping...")
    start_time = time.time()

    # Тормозим, если 50 деревьев подряд не дают улучшений на X_val
    callbacks = [early_stopping(stopping_rounds=50), log_evaluation(0)]

    # --- НИЖНЯЯ ГРАНИЦА (Честные 5%) ---
    model_lower = LGBMRegressor(objective='quantile', alpha=0.02, **lgbm_params)
    ...
    model_upper = LGBMRegressor(objective='quantile', alpha=0.97, **lgbm_params)
    ...
    print(f"\n======================================")
    print(f"=== РЕЗУЛЬТАТЫ (УМНЫЙ КОРИДОР)     ===")
    print(f"======================================")
    ...
    axes[0].set_title('Предсказание границ шума (Ослабленные кандалы)', fontsize=14)
