...
# Корректный импорт изотонической регрессии
...
OUTPUT_MODEL_FILE = "final_model_isotonic.joblib"
...
class CalibratedProductionModel:
    ...
    def predict(self, X_raw):
        ...
        # 2. Прогоняем через изотоническую регрессию
        # Для IsotonicRegression в sklearn метод predict принимает 1D массив
...
def main():
    ...
    # 3. Обучение IsotonicRegression
    print("📏 Настраиваем изотоническое растяжение...")
    y_raw_cal = base_model.predict(X_cal)

    # Создаем и обучаем калибратор
    # out_of_bounds='clip' не дает прогнозу улетать в космос вне изученных данных
    iso_reg = IsotonicRegression(out_of_bounds='clip')
    iso_reg.fit(y_raw_cal, y_cal)

    # 4. Оценка результатов
    print("🧪 Проверка на тестовых данных...")
    y_raw_test = base_model.predict(X_test)
    y_final_test = iso_reg.predict(y_raw_test)
    ...
    print(f"📈 RMSE ПОСЛЕ (Isotonic): {rmse_final:.6f}")
    ...
    final_model_obj = CalibratedProductionModel(base_model, iso_reg)
    ...
    plt.scatter(y_test, y_final_test, s=2, alpha=0.1, color='blue', label='Isotonic (Stretched)')
    plt.plot([y.min(), y.max()], [y.min(), y.max()], 'r--', lw=2, label='Ideal')
    plt.title("Real vs Predicted: Stretching Effect")
    ...
    sns.kdeplot(y_final_test - y_test, color='blue', fill=True, label='Error: Isotonic')
