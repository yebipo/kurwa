...
def train_quantile_models(X_train, y_train, X_test, y_test):
    ...
    plt.title('Предсказание границ шума осциллятора (Вектор из 3 прошлых шагов)')
...
# ==========================================
# 4. ЗАПУСК И МАГИЯ АВТОРЕГРЕССИИ (prev_entropy 1, 2, 3)
# ==========================================
if __name__ == "__main__":
    ...
    def prepare_sequential_data(df, feature_cols, target_col):
        ...
        # 🚀 СМОТРИМ В ПРОШЛОЕ НА 3 ШАГА!
        X['prev_entropy_1'] = y.shift(1)
        X['prev_entropy_2'] = y.shift(2)
        X['prev_entropy_3'] = y.shift(3)

        # Удаляем мусор и первые ТРИ строки (т.к. у них нет полной истории)
        valid_idx = ~y.isna() & ~X['prev_entropy_1'].isna() & ~X['prev_entropy_2'].isna() & ~X['prev_entropy_3'].isna()
        X = X[valid_idx]
        y = y[valid_idx]

        # Переименовываем колонки для порядка (теперь их 7!)
        cols = ['f1', 'f2', 'f3', 'f4', 'prev_entropy_1', 'prev_entropy_2', 'prev_entropy_3']
        X.columns = cols

        return X, y


    print("-> Очистка данных и создание лагов на 3 шага назад...")
    ...
    print(f"Размер Train: {X_train.shape[0]} строк (используем 7 фичей)")
