...
def train_quantile_models(X_train, y_train, X_test, y_test):
    ...
    axes[0].set_title('Предсказание границ шума (Лаги + Окна)', fontsize=14)
    ...
    axes[1].set_xlabel('Условный вес (кол-во использований в деревьях)')
    axes[1].set_ylabel('')
...
if __name__ == "__main__":
    ...
    def prepare_sequential_data(df, feature_cols, target_col):
        ...
        # 1. Вектор прошлого (3 шага)
        X['prev_1'] = y.shift(1)
        X['prev_2'] = y.shift(2)
        X['prev_3'] = y.shift(3)

        # 2. Физика: Скользящие окна (тренд и волатильность)
        X['roll_mean'] = y.rolling(window=10).mean().shift(1)
        X['roll_std'] = y.rolling(window=10).std().shift(1)

        # Очистка мусора (убираем NaN, появившиеся из-за shift и rolling)
